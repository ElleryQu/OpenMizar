...
template<typename T>
__global__ void repeat(T *b, T *expanded,
        int bSize, int expandedSize, int expansionFactor) {
    ...
    if (B_IDX <= bSize && EXP_IDX < expandedSize) {
        int destIdx = (B_IDX * expansionFactor) + EXP_IDX;
        expanded[destIdx] = b[B_IDX];
    }
}
...
template<typename T>
void repeat(DeviceData<T> &b, DeviceData<T> &expanded) {

    int expansionFactor = (expanded.size() / b.size());

    int cols = b.size();
    int rows = expansionFactor;
    ...
    kernel::repeat<<<blocksPerGrid,threadsPerBlock>>>(
        thrust::raw_pointer_cast(&b.begin()[0]),
        thrust::raw_pointer_cast(&expanded.begin()[0]),
        b.size(),
        expanded.size(),
