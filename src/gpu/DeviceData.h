...
class DeviceDataBase {
    ...
    public:
        ...
        DeviceDataBase<T, Iterator> &operator=(const DeviceDataBase<T, I2> &rhs) {
            thrust::copy(this->begin(), this->end(), rhs.begin());
            return *this;
        }
        ...
        DeviceDataBase<T, Iterator> &operator^=(const T rhs) {
            thrust::transform(begin(), end(), begin(), scalar_xor_functor<T>(rhs));
            return *this;
        }

        DeviceDataBase<T, Iterator> &operator&=(const T rhs) {
            thrust::transform(begin(), end(), begin(), scalar_and_functor<T>(rhs));
            return *this;
        }
...
class DeviceData : public DeviceDataBase<T, I> {
    ...
    public:
        ...
        template<typename I2>
        DeviceData<T, I> &operator=(const DeviceData<T, I2> &rhs) {
            DeviceDataBase<T, I>::operator=(rhs);
            return *this;
        }
...
class DeviceData<T, BufferIterator<T> > : public DeviceDataBase<T, BufferIterator<T> > {
    ...
    public:
        ...
        template <typename intT, std::enable_if_t<std::is_integral<intT>::value, bool> = true>
        DeviceData(std::initializer_list<intT> il) : data(il.size()) {
        ...
        DeviceData(std::initializer_list<floatT> il) : data(il.size()) {
            std::vector<floatT> shifted_vals(il.size());
            for (double f : il) {
                shifted_vals.push_back((T) (f * (1 << FLOAT_PRECISION)));
            }

            thrust::copy(shifted_vals.begin(), shifted_vals.end(), data.begin());
            this->set(data.begin(), data.end()); 

            memory_profiler.track_alloc(shifted_vals.size() * sizeof(T));
        }
...
void apply(DeviceData<T, I> &lhs, Func func) {
    thrust::transform(lhs.begin(), lhs.end(), lhs.begin(), func);
}

template<typename T, typename I1, typename I2, typename Func>
void apply(DeviceDataBase<T, I1> &lhs, const DeviceDataBase<T, I2> &rhs, Func func) {
    thrust::transform(lhs.begin(), lhs.end(), rhs.begin(), lhs.begin(), func);
}

// TODO: move this into DeviceDataBase.
template<typename T, typename I1, typename I2>
void logic_rshift(const DeviceData<T, I1> &input, DeviceData<T, I2> &result, T shift) {
    thrust::transform(input.begin(), input.end(), result.begin(), scalar_logic_rshift_functor<T>(shift));
};

template<typename T, typename I1, typename I2, typename I3>
void logic_rshift(const DeviceData<T, I1> &input, DeviceData<T, I2> &result, const DeviceData<T, I3> &shift) {
    thrust::transform(input.begin(), input.end(), shift.begin(), result.begin(), logic_rshift_functor<T>());
};
