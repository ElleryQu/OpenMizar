...
template<typename T>
struct scalar_logic_rshift_functor {

    const T a;

    scalar_logic_rshift_functor(T _a) : a(_a) {}
    __host__ __device__ T operator()(const T &x) const {
        return x >> a;
    }
};
...
template<typename T>
struct logic_rshift_functor {
    logic_rshift_functor() {}
    __host__ __device__ T operator()(const T &x, const T &y) const {
        return x >> y;
    }
};
...
struct field_restruct_functor {    
    ...
    __host__ __device__ T operator()(const T &x) const {
        return static_cast<T>(x - q * (x > static_cast<T>(q-1) / 2));
    }
};
